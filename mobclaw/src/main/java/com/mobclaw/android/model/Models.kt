 ...
 *
 * [imageDataUrl] is deliberately transient so session exports do not duplicate
 * multi-megabyte screenshot payloads. Providers can still read it at runtime.
 */
@Serializable
data class ChatMessage(
    val role: String,
    val content: String,
    @Transient val imageDataUrl: String? = null,
) {
    companion object {
        fun system(content: String) = ChatMessage("system", content)
        fun user(content: String, imageDataUrl: String? = null) = ChatMessage("user", content, imageDataUrl)
